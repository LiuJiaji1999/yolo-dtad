...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, gt_bboxes, gt_cls):
        ...
        if gt_cls.shape[0] == 0:  # Check if labels is empty
            ...
            return
        if detections is None: # 没检测到任何目标
...
def plot_pr_curve(px, py, ap, save_dir=Path("pr_curve.png"), names=(), on_plot=None):
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            ...
            ## 汇总所有pr曲线到一张图上
            ####### start  ##########
            # with open(f'PR-curve/dtadh/{names[i]}.csv','w+') as f:
...
def compute_ap(recall, precision):
    ...
    if method == "interp":
        x = np.linspace(0, 1, 101)  # 101-point interp (COCO)， 首先是在p r列表左右两侧各插了0 1两个值，
        ap = np.trapz(np.interp(x, mrec, mpre), x)  # integrate ，之后通过np.interp 插101个点，最后通过np.trapz 积分得到最终的ap，
    else:  # 'continuous'
