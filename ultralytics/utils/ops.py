...
def uda_non_max_suppression(prediction,
    ...
    Args:
        prediction (torch.Tensor): A tensor of shape (batch_size, num_boxes, num_classes + 4).
        conf_thres (float): The confidence threshold.
        iou_thres (float): The IoU threshold for NMS.
        classes (List[int]): List of class indices to consider.
        agnostic (bool): If True, ignores class information during NMS.
        multi_label (bool): If True, allows multiple labels per box.
        labels (List[List[int]]): A list of apriori labels.
        max_det (int): Maximum number of boxes to retain after NMS.
        nc (int): Number of classes.
        max_nms (int): Maximum number of boxes for NMS.
        max_wh (int): Maximum box width and height.

    Returns:
        (List[torch.Tensor]): A list of detected boxes for each image.
    """
    # Checks
    assert 0 <= conf_thres <= 1, f"Invalid confidence threshold {conf_thres}, valid values are between 0.0 and 1.0"
    ...
    xc = prediction[..., 4] > conf_thres  # shape: (batch_size, num_boxes)
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        x = x[xc[xi]]  # apply confidence threshold on boxes for this image

        # Add apriori labels if available
        ...
        # Split into boxes, class confidences
        box, cls = x[:, :4], x[:, 5:5+nc]
